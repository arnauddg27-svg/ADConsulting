/**
 * FSP ERP → BigQuery Sync — Google Cloud Function
 *
 * Triggered by Cloud Scheduler via HTTP POST.
 * Pulls all data from the Terravista ERP external API and writes to BigQuery.
 *
 * Env vars (set in Cloud Function config):
...
async function truncateLoad(table, rows, schema, dataset = RAW_DATASET) {
  ...
  try {
    ...
    if (metadata.status.errorResult) throw new Error(`Load ${table}: ${metadata.status.errorResult.message}`);
...
function transformSchedule(projects) {
  return projects.map((p) => ({
    job_id: p.id, lot_id: p.lotNumber, job_number: p.jobNumber,
    project_address: p.address, short_address: p.shortAddress,
    community: p.division?.name || null, plan_name: p.planName, plan_id: p.planId,
    phase: p.phase, current_stage: null, start_date: ts(p.startDate),
    progress_percent: num(p.progressPercent ?? p.progress_percent ?? p.milestoneProgress?.percent ?? p.milestone_progress?.percent),
    progress_milestone: p.progressMilestone ?? p.progress_milestone ?? p.milestoneProgress?.current ?? p.milestone_progress?.current ?? null,
    progress_matched_task_name: p.milestoneProgress?.matchedTaskName ?? p.milestone_progress?.matched_task_name ?? null,
    ...
    permit_number: p.permitNumber, status: p.status, sales_status: p.salesStatus,
    division_id: p.divisionId, division_code: p.division?.divisionCode || null,
    _extracted_at: new Date().toISOString(), _source_erp: "fsp",
  }));
}

function transformTasks(tasks) {
  return tasks.map((t) => ({
    task_id: t.id, task_number: t.taskNumber,
    lot_id: t.project?.id || null, job_id: t.project?.id || null,
    job_name: t.project?.jobNumber || null, project_address: t.project?.address || null,
    community: null, plan_name: t.project?.planName || null,
    stage: t.stage, task_group: t.taskGroup, activity: t.activity,
    cost_code: t.costCode, vendor_name: t.vendor?.name || null,
    vendor_id: t.vendor?.id || null, status: t.status, draw_status: t.drawStatus,
    is_variable: t.isVariable || false, cost_overridden: t.costOverridden || false,
    scheduled_start: ts(t.schedStart), scheduled_end: ts(t.schedEnd),
    actual_start: ts(t.actualStart), actual_end: ts(t.actualEnd),
    budget_cost: num(t.budgetCost), actual_cost: num(t.actualCost),
    duration: num(t.duration), notes: t.notes,
    _extracted_at: new Date().toISOString(), _source_erp: "fsp",
  }));
}
...
function transformVendorsPO(purchaseOrders) {
  return purchaseOrders.map((po) => ({
    vendor_id: po.vendor?.id || null, vendor_name: po.vendor?.name || null,
    po_number: po.poNumber, po_id: po.id,
    job_id: po.project?.id || null, task_id: po.task?.id || null,
    task_number: po.task?.taskNumber || null, activity: po.task?.activity || null,
    cost_code: po.task?.costCode || null, stage: po.task?.stage || null,
    task_group: po.task?.taskGroup || null,
    po_amount: num(po.amount), invoiced_amount: null,
    is_epo: po.isEPO || false, status: po.orderStatus,
    draw_status: po.drawStatus, is_paid: po.drawStatus === "PAID",
    issue_date: ts(po.sentAt), paid_at: ts(po.paidAt), description: po.description,
    _extracted_at: new Date().toISOString(), _source_erp: "fsp",
  }));
}
...
function transformVendorsMaster(vendors) {
  return vendors.map((v) => ({
    vendor_id: v.id, vendor_name: v.name, vendor_number: v.vendorNumber,
    short_name: v.shortName, contact_name: v.contactName,
    address: v.address, city: v.city, state: v.state, zip: v.zip,
    email: v.email, phone: v.phone, trade: v.trade,
    task_count: v._count?.tasks || 0, po_count: v._count?.purchaseOrders || 0,
    _extracted_at: new Date().toISOString(), _source_erp: "fsp",
  }));
}

function transformCostSummaries(costSummaries, projects) {
  return costSummaries.filter(Boolean).map((cs) => {
    const d = cs.data || cs;
    ...
    return {
      ...
      task_count: num(s.counts?.tasks), po_count: num(s.counts?.purchaseOrders),
...
async function rebuildMarts() {
  await runQuery(`
    ...
    SELECT vendor_id AS vendor_scorecard_key, vendor_id, vendor_name,
      SAFE_DIVIDE(SUM(CASE WHEN po_amount > 0 THEN po_amount ELSE 0 END) - SUM(CASE WHEN is_paid THEN po_amount ELSE 0 END), NULLIF(SUM(po_amount), 0)) AS avg_variance_pct,
    ...
    FROM ${raw("vendors")} WHERE status != 'CANCELLED'
    GROUP BY vendor_id, vendor_name ORDER BY ranking
  ...
  await runQuery(`
    ...
    SELECT * FROM (
      SELECT CONCAT('no_vendor_', task_id) AS exception_center_key, 'TASK_NO_VENDOR' AS exception_type,
        'MEDIUM' AS severity, CONCAT(COALESCE(project_address, job_id), ' / ', activity) AS affected_entity, 0 AS days_outstanding
      FROM ${raw("schedule_tasks")} WHERE vendor_name IS NULL AND budget_cost > 0
      UNION ALL
      SELECT CONCAT('orphan_po_', po_id), 'PO_NO_TASK', IF(is_epo, 'LOW', 'HIGH'),
        CONCAT(vendor_name, ' / PO#', po_number), DATE_DIFF(CURRENT_DATE(), DATE(issue_date), DAY)
      FROM ${raw("vendors")} WHERE task_id IS NULL AND status != 'CANCELLED' AND NOT is_epo
      UNION ALL
      SELECT CONCAT('overdue_', task_id), 'TASK_OVERDUE',
        IF(DATE_DIFF(CURRENT_DATE(), DATE(scheduled_end), DAY) > 14, 'HIGH', 'MEDIUM'),
        CONCAT(COALESCE(project_address, job_id), ' / ', activity),
        DATE_DIFF(CURRENT_DATE(), DATE(scheduled_end), DAY)
      FROM ${raw("schedule_tasks")} WHERE status NOT IN ('COMPLETED') AND scheduled_end IS NOT NULL AND DATE(scheduled_end) < CURRENT_DATE()
    )
  `, "mart_exception_center");

  await runQuery(`
    CREATE OR REPLACE TABLE ${mart("mart_job_profitability")} AS
    SELECT CONCAT(t.job_id, '_', COALESCE(t.cost_code, 'NONE')) AS job_profitability_key,
      t.job_id, MAX(t.project_address) AS job_name, MAX(s.community) AS community, t.cost_code,
      MAX(t.activity) AS cost_code_description,
      SUM(COALESCE(t.budget_cost, 0)) AS budgeted_amount, SUM(COALESCE(t.actual_cost, 0)) AS actual_amount,
      COALESCE(po_agg.committed, 0) AS committed_amount,
      SUM(COALESCE(t.budget_cost, 0)) - COALESCE(po_agg.committed, 0) AS variance,
      SAFE_DIVIDE(COALESCE(po_agg.committed, 0) - SUM(COALESCE(t.budget_cost, 0)), NULLIF(SUM(COALESCE(t.budget_cost, 0)), 0)) AS variance_pct,
    ...
    FROM ${raw("schedule_tasks")} t
    LEFT JOIN ${raw("schedule")} s ON t.job_id = s.job_id
    LEFT JOIN (SELECT job_id, cost_code, SUM(po_amount) AS committed FROM ${raw("vendors")} WHERE status != 'CANCELLED' GROUP BY job_id, cost_code) po_agg
      ON t.job_id = po_agg.job_id AND t.cost_code = po_agg.cost_code
...
functions.http("fspErpSync", async (req, res) => {
  const start = Date.now();

  if (!API_KEY) {
    res.status(500).json({ error: "Missing FSP_API_KEY env var" });
    return;
  }
  ...
  try {
    ...
    const costSummaries = await Promise.all(
      projects.map((p) => apiFetch(`/api/external/projects/${p.id}/cost-summary`).catch(() => null))
    );

    console.log(`Projects: ${projects.length}, Tasks: ${tasks.length}, POs: ${purchaseOrders.length}, Vendors: ${vendors.length}`);

    // Transform
    const scheduleRows = transformSchedule(projects);
    const taskRows = transformTasks(tasks);
    const vendorPORows = transformVendorsPO(purchaseOrders);
    const vendorMasterRows = transformVendorsMaster(vendors);
    const spendingRows = transformCostSummaries(costSummaries, projects);

    // Load raw tables
    console.log("Writing raw tables...");
    ...
    const result = {
      status: "ok",
      elapsed: `${elapsed}s`,
      counts: { projects: projects.length, tasks: tasks.length, purchaseOrders: purchaseOrders.length, vendors: vendors.length },
    };

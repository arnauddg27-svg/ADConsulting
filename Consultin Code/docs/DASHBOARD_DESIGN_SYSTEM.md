...
> architecture and design-token summary, see
> **[`DASHBOARD_KPI_REFERENCE.md`](./DASHBOARD_KPI_REFERENCE.md)**.
> This file remains for detailed design-token narrative. The live tokens are in
> `src/components/demo/sunshine/sunshine-tokens.css` (sample) or
> `clients/{client}/app/globals.css` (per-client).

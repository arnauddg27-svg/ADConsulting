...
function generateAuditJobs(): SHAuditJob[] {
  ...
  for (let i = 0; i < auditableJobs.length; i++) {
    ...
    const vertical = rng.between(165000, 220000);
    const options = rng.between(0, 15000);
    const dirtPad = rng.between(2500, 12000);
    const dumpsters = rng.between(3000, 7000);
    const financing = rng.between(2000, 60000);
    const insurance = rng.between(0, 4000);
    const closingCost = rng.between(800, 8000);
    const well = rng.between(0, 3500);
    const septic = rng.between(0, 20000);
    const waterFiltration = rng.between(0, 3500);
    const gopherTortoise = rng.between(0, 500);
    const treeSurvey = rng.between(0, 350);
    const builderFeePct = rng.between(5, 10) / 100;

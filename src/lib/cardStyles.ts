 ...
 * - a hover lift + deeper shadow so every card responds to the pointer
 ...
 * backdrop / transition / hover). Call sites should pass only layout +
 * spacing classes via `className` and must not re-declare a conflicting
 * radius/border/bg/shadow.
 */

const PANEL_BASE =
  "relative min-w-0 rounded-[1.5rem] border border-[#d4dee4] transform-gpu " +
  ...
  "hover:-translate-y-1 hover:border-[#c6d3da] " +
  "hover:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.96),0_6px_14px_-6px_rgba(23,33,44,0.10),0_44px_88px_-46px_rgba(23,33,44,0.5)] " +
  "motion-reduce:transition-none motion-reduce:hover:translate-y-0";

const TILE_BASE =
  "relative rounded-xl border border-[#dbe3e8] transform-gpu " +
...
export function glassTileClass({
  ...
  return clsx(TILE_BASE, className);
}

  ...
  return (
    <>
      ...
      <div className="sh-kpi-row">
        ...
        <SHKpiCard label="Completions" value={fmtN(completionsThisPeriod)} sub="COs received" accent="#0f766e" sparkline={[3, 4, 5, 4, 6, 5, 7, 6, 8, completionsThisPeriod]} onClick={() => onDrill({ type: "cycle-metric", value: "completions", label: `Completions — ${fmtN(completionsThisPeriod)}` })} />
        <SHKpiCard label="In Construction" value={fmtN(activeCount)} sub="Active jobs" accent="#3b82f6" progress={Math.round((activeCount / Math.max(jobs.length, 1)) * 100)} onClick={() => onDrill({ type: "cycle-metric", value: "in-construction", label: `In Construction — ${fmtN(activeCount)}` })} />
      </div>
